...
function AdminLMIAPayments() {
  ...
  const loadPayments = async () => {
    try {
      ...
      const paymentsRef = collection(db, "lmiaVerifyPayments");
      ...
      console.error("Error loading LMIA payments:", err);
      setError("Failed to load LMIA payments: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateStatus = async (paymentId, newStatus) => {
    try {
      ...
      const paymentRef = doc(db, "lmiaVerifyPayments", paymentId);
  ...
  const getServiceTypeVariant = (serviceType) => {
    switch (serviceType) {
      case "lmia_verification": return "primary";
      case "lmia_application": return "info";
      case "lmia_renewal": return "warning";
      case "lmia_consultation": return "success";
    ...
    return (
      <Container className="py-4">
        <div className="text-center">
          ...
          <p className="mt-3">Loading LMIA Payments...</p>
        </div>
      </Container>
    );
  }
              ...
              <Row className="align-items-center">
                <Col>
                  <h4 className="mb-1 text-dark">
                    ...
                    LMIA Payment Management
                  </h4>
                  <p className="mb-0 text-muted">
                    Manage LMIA payment transactions and verifications
                  </p>
                </Col>
                <Col xs="auto">
      ...
      <Row className="mb-4">
        <Col md={6}>
          <InputGroup>
            ...
            <Form.Control
              type="text"
              placeholder="Search by email, transaction ID, payment ID, method..."
              value={searchTerm}
          ...
          <Card className="border-0 shadow-sm">
            <Card.Header className="bg-white py-3">
              <h5 className="mb-0">
                ...
                LMIA Payments ({filteredPayments.length})
              </h5>
            </Card.Header>
                    ...
                    {filteredPayments.length === 0 ? (
                      ...
                      filteredPayments.map((payment) => (
                        <tr key={payment.id}>
                          ...
                          <td>
                            <Badge bg={getServiceTypeVariant(payment.serviceType)}>
...
export default AdminLMIAPayments;